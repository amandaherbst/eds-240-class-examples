...
jobs_clean %>% 
  filter(year == 2016) %>% 
  # top ten highest earning occuptations
...
- truncated axes lead viewers to perceive illustrated differences as larger or more important than they actually are (i.e. a truncation effect)

**Bar (and related) plots are for categorical data**

- aboid using to visualize continuous data -- can hide distributions

### lollipop variant: dumbell plot

simple and intuitive way to visualize a change or difference in two sets of data points. Combo of `geom_segment` and `geom_point`
...
ggplot(subset_jobs) +
  ...
  # scales = "free_y" only plots the occupations that have data for each facet
